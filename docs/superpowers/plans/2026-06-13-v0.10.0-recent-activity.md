...
|------|------|-------|-----------------------|
| [ ] | Task 97 | 活动 migration、类型和清理规则 | `cargo test recent_activity_tests`; `cargo check` |
| [ ] | Task 98 | 聚合筛选与分页 repository | `cargo test recent_activity_tests`; `cargo check` |
| [ ] | Task 99 | Tauri command 与 TypeScript 契约 | `cargo check`; `npm run build` |
| [ ] | Task 100 | 最近筛选侧栏与时间线组件 | focused frontend tests; `npm run build` |
| [ ] | Task 101 | App 状态串联和旧最近网格移除 | `npm test`; `npm run build` |
...
- [ ] **Step 2: Write failing sidebar filter tests**
...
- [ ] **Step 3: Implement timeline contract**
...
- [ ] **Step 4: Implement recent sidebar filters**
...
- [ ] **Step 5: Add empty state variants**
...
- [ ] **Step 3: Run metadata and full verification**
...
Mark Tasks 97-102 complete only after their required verification passes.
